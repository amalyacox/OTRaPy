...
class RamanSolver: 
    ...
    def Txy(self, kx:float=62.2, ky:float=62.2, g:float=1.94E6, Q:float=1e-3, threshold:float=1.E-5, Ta:float=300., d2h=None, verbose:bool=False):
        """
        ...
        while error > threshold: 
            ...
            T = (kx * (np.roll(T, -1, axis=1) + np.roll(T, 1, axis=1)) + ky * (np.roll(T, -1, axis=0) + np.roll(T,1,axis=0) + d2h * (self.qdot + g * Ta))) / (d2h * g + 2*kx + 2*ky) 
