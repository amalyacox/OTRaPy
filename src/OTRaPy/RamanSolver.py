...
class RamanSolver:
    ...
    def __init__(
        ...
        self.w0_dTdQg, self.l0_dTdQg = (
            w0_dTdQg,
            l0_dTdQg,
        )  # gaussian beam width for isotropic measurement
        self.w0_dTdQx, self.l0_dTdQx, self.w0_dTdQy, self.l0_dTdQy = (
            w0_dTdQx,
            l0_dTdQx,
            w0_dTdQy,
            l0_dTdQy,
        )  # gaussian beam width for anisotropic measurements
    ...
    def analytic_Txy(self, kx=62.2, ky=62.2, g=1.94e6, Q=1e-3, Ta=300, upper=10e-6):
        ...
        Ta : float
            Temperature of substrate (K), default 300
        upper : float
            Upper limit of integration, default 10e-6 (m)

        Returns
        ---------
        T : array-like
            Temperature distribution in the material (K)
        """
        T = []
        ...
        for r in rr:
            ...
            sol = (
                const
                * integrate.quad(integrand_T, 0, upper, args=(r, kx, g, r0, self.h))[0]
            )
            T.append(sol)
        T = np.array(T)
    ...
    def weighted_average_analytic(
        self, k=62.2, g=1.94e6, Q=1e-3, ratio=None, upper=10e-6
    ):
        """
        Compute the weighted average temperature measured by raman using analytic solution to cylindrical heating

        Parameters
        ----------
        k : float
            Thermal conductivity in x- and y-direction (isotropic)
        g : float
            Interfacial thermal conductance
        Q : float
            Input laser power, default 1e-3(W)
        ratio : float, optional
            ratio of k/g, default None
        ...
        def integrand(r, Q, k, h, g, ratio, r0=r0):
            ...
            if ratio is not None:
                ...
                T = (
                    const
                    * integrate.quad(
                        integrand_T_solve, 0, upper, args=(r, ratio, r0, self.h)
                    )[0]
                )
            else:
                T = (
                    const
                    * integrate.quad(integrand_T, 0, upper, args=(r, k, g, r0, self.h))[
                        0
                    ]
                )
        ...
        if ratio is not None:
            Tav = (
                integrate.quad(integrand, 0, 10e-6, args=(Q, k, self.h, g, ratio))[0]
                * 1
                / (r0**2)
            )
        else:
            Tav = integrate.quad(integrand, 0, 10e-6, args=(Q, k, self.h, g, ratio))[
                0
            ] / (r0**2 / 2)
        return Tav
    ...
    def dTdQ_sim(
        self, kx: float, ky: float, g: float, Q, w0=None, l0=None, alpha=None, Ta=300
    ):
        """
    ...
    def resid_dTdQ(
        self, kx, ky, g, Q, dT_arr, dT_err, w0=None, l0=None, alpha=None, Ta=300, off=0
    ):
        """
    ...
    def analytic_loss(self, ratio, dTdQ_r1, dTdQ_r2, r1, r2):
        """
        Loss function for minimizing analytic solution
        to get ratio of k/g from experimental data

        Parameters
        ----------
        ratio : float
            ratio of k/g to be solved for
        dTdQ_r1 : float
            experimental dTdQ for r1
        dTdQ_r2 : float
            experimental dTdQ for r2
        r1 : float
            laser spot size for r1
        r2 : float
            laser spot size for r2
    ...
    def get_ratio(self, dTdQ_1, dTdQ_2, r1, r2, x0, **ls_kwargs):
        """
        Get ratio of k/g from experimental data using analytic solution.
        Minimize self.analytic_loss
        ...
        r2 : float
            laser spot size for r2
        x0 : float
            initial guess for ratio of k/g
        **ls_kwargs : keyword arguments
            optional arguments for least_squares function
        """
        root = minimize(
            self.analytic_loss, x0=x0, args=(dTdQ_1, dTdQ_2, r1, r2), **ls_kwargs
        )
    ...
    def solve_k_from_ratio(self, k, ratio, r, dTdQ, Q, verbose=False):
        """
        Use the found ratio to solve for k and g using one dTdQ function and spot size
        ...
        ratio : float
            ratio of k/g to be solved for
        r : float
            laser spot size for solving
        dTdQ : float
            experimental dTdQ for solving; corresponds with r
        Q : array-like
            Range of input power values
        verbose : bool, optional
            if True, print the residual for debugging, default False
        """
        g = k * ratio
        self.update_w0(r)
        self.update_l0(r)
    ...
    def get_props_analytic(
        self, x0, ratio, r1, r2, dTdQ_1, dTdQ_2, Q, verbose=False, **ls_kwargs
    ):
        """
        Get analytic solutions to k and g using the found ratio from get_ratio and two dTdQ experimental slopes and spot sizes
        ...
        ratio : float
            ratio of k/g to be solved for
        r1 : float
            laser spot size for solving k1
        r2 : float
            laser spot size for solving k2
        dTdQ_1 : float
            experimental dTdQ for solving k1
        dTdQ_2 : float
            experimental dTdQ for solving k2
        Q : array-like
            Range of input power values
        verbose : bool, optional
            if True, print the residual for debugging, default False
        **ls_kwargs : keyword arguments
        ...
        if verbose:
            print("solving k1")
        root_k1 = least_squares(
            self.solve_k_from_ratio,
            x0=x0,
            args=(ratio, r1, dTdQ_1, Q, verbose),
            **ls_kwargs,
        )
        if verbose:
            print("solving k2")
        root_k2 = least_squares(
            self.solve_k_from_ratio,
            x0=x0,
            args=(ratio, r2, dTdQ_2, Q, verbose),
            **ls_kwargs,
        )
    ...
    def resid_full(
        ...
        eq1 = self.resid_dTdQ(
            kx, ky, g, Qx, dTdQ_x_arr, dTdQ_x_err, w0_dTdQx, l0_dTdQx, alpha_x, Ta, offx
        )
        eq2 = self.resid_dTdQ(
            kx, ky, g, Qy, dTdQ_y_arr, dTdQ_y_err, w0_dTdQy, l0_dTdQy, alpha_y, Ta, offy
        )
        eq3 = self.resid_dTdQ(
            kx, ky, g, Qg, dTdQ_g_arr, dTdQ_g_err, w0_dTdQg, l0_dTdQg, alpha_g, Ta, offg
        )
    ...
    def resid_fixg(
        ...
        kx, ky = p
        eq1 = self.resid_dTdQ(
            kx, ky, g, Qx, dTdQ_x_arr, dTdQ_x_err, w0_dTdQx, l0_dTdQx, alpha_x, Ta
        )
        eq2 = self.resid_dTdQ(
            kx, ky, g, Qy, dTdQ_y_arr, dTdQ_y_err, w0_dTdQy, l0_dTdQy, alpha_y, Ta
        )
        eqtns = np.concatenate([eq1, eq2])
        print(np.sum(eqtns), kx, ky)
        return eqtns

    def minimize_resid_fixg(
        self,
        x0,
        dTdQ_x_arr,
        dTdQ_x_err,
    ...
    def resid_fixk(
        self, p, dTdQ_g_arr, dTdQ_g_err, Qg, w0_dTdQg, l0_dTdQg, alpha_g, kx, ky, Ta
    ):
        """
        ...
        g = p
        eq1 = self.resid_dTdQ(
            kx, ky, g, Qg, dTdQ_g_arr, dTdQ_g_err, w0_dTdQg, l0_dTdQg, alpha_g, Ta
        )
        # print(np.sum(eq1), g)
        return eq1

    def minimize_resid_fixk(
        self,
        x0,
        dTdQ_g_arr,
        dTdQ_g_err,
        ...
        **ls_kwargs,
        ...
        root = least_squares(
            self.resid_fixk,
            x0=x0,
            args=(dTdQ_g_arr, dTdQ_g_err, Qg, w0_dTdQg, l0_dTdQg, alpha_g, kx, ky, Ta),
            **ls_kwargs,
        )
        return root

    def resid_full_varyall(
        self,
        p,
        dTdQ_x_arr,
        dTdQ_x_err,
    ...
    def minimize_resid_full_varyall(
        self,
        x0,
        dTdQ_x_arr,
        dTdQ_x_err,
    ...
    def solve_kx(
        ...
        ky: float,
        g: float,
        w0_dTdQx: float = 0.4e-6,
        l0_dTdQx: float = 0.4e-6,
        verbose: bool = False,
    ):
        """
    ...
    def solve_ky(
        ...
        kx: float,
        g: float,
        w0_dTdQy: float = 0.4e-6,
        l0_dTdQy: float = 0.4e-6,
        alpha=None,
        verbose: bool = False,
    ):
        """
    ...
    def solve_g(
        ...
        kx: float,
        ky: float,
        w0_dTdQg: float = 0.4e-6,
        l0_dTdQg: float = 0.4e-6,
        alpha=None,
        verbose: bool = False,
    ):
        """
    ...
    def solve_anisotropic_raman_fixg(
        self,
        p,
        g,
        dTdQ_x,
    ...
    def get_kx_ky_fixg(
        ...
        **ls_kwargs,
        ...
        root = least_squares(
            self.solve_anisotropic_raman_fixg,
            x0=x0,
            args=(
                g,
                dTdQ_x,
    ...
    def solve_anisotropic_raman_varyall(self, p, dTdQ_x, Qx, dTdQ_y, Qy, dTdQ_g, Qg):
        ...
        kx, ky, g, h, alpha_x, alpha_y, alpha_g, w0_dTdQx, l0_dTdQx, w0_dTdQg = (
            p  # w0_dTdQy, l0_dTdQy,
        )
        w0_dTdQy = (
            l0_dTdQx  # enforcing anisotropic beam to be the same for both measurements
        )
        l0_dTdQy = w0_dTdQx
    ...
    def get_kx_ky_g_varyall(self, x0, dTdQ_x, Qx, dTdQ_y, Qy, dTdQ_g, Qg, **ls_kwargs):
        ...
        root = least_squares(
            self.solve_anisotropic_raman_varyall,
            x0=x0,
            args=(
                dTdQ_x,
                Qx,
    ...
    def get_kx_ky_varyall(self, x0, dTdQ_x, Qx, dTdQ_y, Qy, g, **ls_kwargs):
        ...
        root = least_squares(
            self.solve_anisotropic_raman_kxky_varyall,
            x0=x0,
            args=(dTdQ_x, Qx, dTdQ_y, Qy, g),
            **ls_kwargs,
        )
        return root
    ...
    def get_k_g_varyall(self, x0, dTdQ_array, Q, **ls_kwargs):
        ...
        """
        root = least_squares(
            self.solve_isotropic_raman_varyall, x0=x0, args=(dTdQ_array, Q), **ls_kwargs
        )
        return root
    ...
    def get_k_g(self, x0, dTdQ_array, Q, **ls_kwargs):
        ...
        """
        root = least_squares(
            self.solve_isotropic_raman, x0=x0, args=(dTdQ_array, Q), **ls_kwargs
        )
        return root
    ...
    def get_g_fixk(self, x0, dTdQ_array, Q, kx, ky, **ls_kwargs):
        ...
        """
        root = least_squares(
            self.solve_isotropic_raman_fixk,
            x0=x0,
            args=(dTdQ_array, Q, kx, ky),
            **ls_kwargs,
        )
        return root
    ...
    def minimize_resid_full_isotropic(self, x0, dT_arrs, Q, Ta, **ls_kwargs):
        root = least_squares(
            self.resid_full_isotropic, x0=x0, args=(dT_arrs, Q, Ta), **ls_kwargs
        )
        return root
    ...
    def minimize_resid_full_isotropic_varyall(self, x0, dT_arrs, Q, Ta, **ls_kwargs):
        root = least_squares(
            self.resid_full_isotropic_varyall, x0=x0, args=(dT_arrs, Q, Ta), **ls_kwargs
        )
        return root
    ...
    def get_kx_ky_g_curve(self, x0, dTdQ_array, Q, **ls_kwargs):
        root = least_squares(
            self.solve_anisotropic_raman_curve, x0=x0, args=(dTdQ_array, Q), **ls_kwargs
        )
        return root

    ####################################################################################################################
    # Ignore for now .. 1/10/25
    def mc_sim_resid_full_varyall(
        self,
        x0,
        dT_x,
        dT_x_err,
        ...
        **ls_kwargs,
        ...
        for i in range(ntrials):
            ...
            pwr_x_trial, dT_x_trial, dT_x_err_trial = (
                Qx[xidx],
                dT_x[xidx],
                dT_x_err[xidx],
            )
            ...
            pwr_y_trial, dT_y_trial, dT_y_err_trial = (
                Qy[yidx],
                dT_y[yidx],
                dT_y_err[yidx],
            )
            ...
            pwr_g_trial, dT_g_trial, dT_g_err_trial = (
                Qg[gidx],
                dT_g[gidx],
                dT_g_err[gidx],
            )

            root = self.minimize_resid_full_varyall(
                x0,
                dT_x_trial,
                dT_x_err_trial,
                pwr_x_trial,
    ...
    def iter_solve(
        ...
        """
        warnings.warn(
            "iter_solve is deprecated, use new_function instead",
            DeprecationWarning,
            stacklevel=2,
        )
