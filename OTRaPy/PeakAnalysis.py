...
def fit_multiple(files):
    ...
    for i, f in enumerate(files): 
        ...
        wls = np.array(file['/measurement/piezo_hyperspec/wls'])
        spec_map = np.array(file['/measurement/piezo_hyperspec/spec_map'])
        pz, px, py, _, pwv = spec_map.shape
        spec_map = spec_map.reshape(px*py, pwv)
        av = spec_map.mean(axis=0)
        E, Eerr, A, Aerr, x, y, out = fit_E_A(wls, av, peakfunction=GaussianModel, backgroundfunction=ExponentialModel)
        Aarr[i] = A 
        Aerrs[i] = Aerr
        Earr[i] = E
        Eerrs[i] = Eerr
